    ...
    class Program
    {
        ...
        static void Load(string path, string name)
        {
            ...
            if (assetsManager.assetsFileList.Count == 0)
                return;
            var containers = new List<AssetStudio.Object>();
            ...
            var baseDestPath = Path.Combine(Path.GetDirectoryName(path), "Live2DOutput");
            ...
            foreach (var asset in containers)
            {
                if (asset is GameObject m_GameObject && m_GameObject.m_Name.Equals(name))
                {
                    gameObjects.Add(m_GameObject);

                    if (m_GameObject.m_Animator != null && m_GameObject.m_Animator.m_Controller.TryGet(out var m_Controller))
                    {
                        Console.WriteLine("Filting AnimationClip...");
                        switch (m_Controller)
                        {
                                    ...
                                    if (m_AnimatorOverrideController.m_Controller.TryGet<AnimatorController>(out var m_AnimatorController))
                                    {
                                        foreach (var pptr in m_AnimatorController.m_AnimationClips)
                                        {
                                            if (pptr.TryGet(out var m_AnimationClip))
                                            {
                                                ...
                                                Console.WriteLine($"Collecting AnimationClip: {m_AnimationClip.m_Name}");
                                            }
                                        }
                                    }
                                    break;
                    ...
                    {
                        Console.WriteLine("AnimatorController Not Fount.");
                    }

                    foreach (var componentPtr in m_GameObject.m_Components)
                    {
                        if (!componentPtr.TryGet(out var m_Component)) continue;

                        if (m_Component is MonoBehaviour m_MonoBehaviour)
                        {
                            monoBehaviours.Add(m_MonoBehaviour);
                        }
                    }
                }
                else if (asset is Texture2D m_Texture2D && m_Texture2D.m_Name.StartsWith("texture_"))
                {
                    texture2Ds.Add(m_Texture2D);
                }
                else if (asset is MonoBehaviour m_MonoBehaviour && m_MonoBehaviour.m_Name.Equals(name))
                {
                    monoBehaviours.Add(m_MonoBehaviour);
                }
            }
